#!/bin/bash
sqplex="sqlite3"
export_db="com.plexapp.plugins.library.db"
...
for j in "${export_id[@]}"; do   
   ...
   if [ ! -z "$foldername" ]; then
     mkdir -p "$foldername"
     filename=$("$sqplex" "$export_db" "SELECT title_sort FROM metadata_items WHERE metadata_type = 15 AND id = ${j};" | sed 's/[^A-Za-z0-9._-]/./g')
     echo $filename
